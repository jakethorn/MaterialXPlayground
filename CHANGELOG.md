...
- ShadingLanguageX code view: the document decompiled to editable SLX code in a panel docked beside the node list. Compile regenerates the node graph from the code (undoable), and Decompile refreshes the code from the graph. The code is syntax highlighted, with suggestions as you type, parameter hints inside calls, and hover cards showing a standard library function's signatures and description. A compile error squiggles the line it's on, and Ctrl/Cmd+clicking a standard library function opens its documentation. Not available in the VS Code extension.
