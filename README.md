...
`npm run vendor` needs network access once; it also fetches the ~20 MB OpenUSD Scene Viewer runtime into `vendor/usd-webview-bindings/`, which only USD stages need: the Scene Viewer also opens glTF, GLB and OBJ files, with their materials converted to MaterialX, and fetches the ShadingLanguageX (`.mxsl`) compiler into `vendor/mxslc/`. If your clone lives inside a OneDrive-synced folder, `npm run vendor` can fail with `EPERM`; clone outside synced folders instead.
`npm run vendor` needs network access once; it also fetches the ~20 MB OpenUSD Scene Viewer runtime into `vendor/usd-webview-bindings/` and the ShadingLanguageX (`.mxsl`) compiler into `vendor/mxslc/`. If your clone lives inside a OneDrive-synced folder, `npm run vendor` can fail with `EPERM`; clone outside synced folders instead.
